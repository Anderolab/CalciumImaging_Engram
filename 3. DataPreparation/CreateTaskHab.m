...
durations(1) = 300;       % FA duration
% durations(2) = input('Tone: ');    % Tone 1 duration
% durations(3) = input('Shock: ');   % Shock 1 duration
% durations(4) = input('ITI: ');     % ITI 1 duration

% Display the results
% fprintf('Durations saved as:\n');
% fprintf('  durations = [%.2f, %.2f, %.2f, %.2f];\n', ...
% durations(1), durations(2), durations(3), durations(4));

%% Inicializar las celdas para almacenar los resultados
task_titles = {};  % Nombres de los periodos (FA, Tone 1, Shock 1, ITI 1, etc.)
...
%     % Shock
%     intervals{end+1} = sprintf('Shock %d', i);
%     % ITI
...
%     elseif mod(i, 3) == 0  % "Shock"
%         duration_sec = durations(3);  % Duración de Shock
%     else  % "ITI"
%         duration_sec = durations(4);  % Duración de ITI (ahora siempre 180 segundos)
%     end
% 
...
task_start = input('Start: ');
task_end = input('End: ');

task_frames = task_end(1) - task_start(1) + 1;
task_durations{end+1} = 300;
task_titles{end+1} = intervals{1};

% Crear la estructura Task
Task = struct();
Task.Titles = task_titles;
Task.Lengths = task_durations;
Task.Frames = task_frames;  % Convertir la celda de frames a matriz
