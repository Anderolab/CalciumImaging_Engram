...
% Expected animals
Animals = AnimalConfig.Animals;
animalsToRemove = input('Enter the animals to remove (e.g., [1 2 5]): ');
Animals(ismember(Animals, animalsToRemove)) = [];
...
for trial_type = TrialTypes
    ...
    for animal = 1:length(Animals)
        ...
        animalName = Animals(animal)

        % Asking the user to provide with task information for each animal
        prompt = sprintf('Select the task mat file for animal %d', animalName);
        ...
        session_name = F_NamingFunction(animalName, trial_type);  % You need to define this function
        relevant_path = path_files(contains(path_files, session_name));

        if isempty(relevant_path)
            fprintf('Session for animal %d and trial %s not found.\n', animalName, trial_type);
            Report.Missing = [Report.Missing; "Session for animal " + animalName + " and trial " + trial_type + " not found."];
        ...
        Experiment.(trial_type).("M" + num2str(animalName)) = struct(...
            'Path', relevant_path, ...
            'Raw', raw_, ...
            'Filt', ms.FiltTraces', ...
            'Task', Task ...
